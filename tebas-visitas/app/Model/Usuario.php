...
//	var $belongsTo=array(
//                    'Perfile'=>array('className' =>'Perfile','foreignKey'=>'perfil_id')/*,
//						'Sucursale'=>array('className' =>'Sucursale','foreignKey'=>'sucursal_id')*/);	
   ...
   var $validate=array('nombre'=>array(
	...
	function validate_data_integrity($data){
		...
		$usuario=$this->find("first",array("conditions"=>array("Usuario.email"=>$data['correo_electronico'],"Usuario.claveDeAceso"=>$data['clave_secreta']),"fields"=>array("Usuario.id", "Usuario.activo","Role.id","Role.titulo", "Empresa.id", "Empresa.razonSocial","Sucursal.id","Sucursal.titulo", "Empleado.nombre", "Empleado.apellidoPaterno", "Empleado.apellidoMaterno")));	

		if(!empty($usuario)):
			if(isset($usuario['Usuario']['activo']) && $usuario['Usuario']['activo'] =="t"):
				$result['id']=$usuario['Usuario']['id'];
				$result['roleId']=$usuario['Role']['id'];				
				$result['roleTitulo']=$usuario['Role']['titulo'];	
                    $result['empresaId']=$usuario['Empresa']['id'];	
                    $result['empresaTitulo']=$usuario['Empresa']['razonSocial'];	
                    $result['sucursalId']=$usuario['Sucursal']['id'];	
                    $result['sucursalTitulo']=$usuario['Sucursal']['titulo'];
                    $result['empleadoNombre']=$usuario['Empleado']['nombre'];
                    $result['empleadoApellidoPaterno']=$usuario['Empleado']['apellidoPaterno'];
                    $result['empleadoApellidoMaterno']=$usuario['Empleado']['apellidoMaterno'];
			else:
